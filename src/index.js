...
function createDefaultTasks() {
  if (myProjects[0].tasks.length === 0) {
    myProjects[0].tasks.push(
      new Todos(
        "Pack my Bag for my trip to Barcelona",
        "Pack my bag for my trip to Barcelona",
        "25/10/2021"
      )
    );
...
function innitTaskButtons(ind) {
  ...
  taskDeleteButtons.forEach((taskButton) =>
  ...
  taskInfoButtons.forEach((taskButton) =>
    taskButton.addEventListener("click", displayInfoTask)
  );
  const tasksList = document.querySelector(".tasks");
  tasksList.addEventListener("click", toggleDone);
  const taskEditButtons = document.querySelectorAll("#edit-icon-id");
  taskEditButtons.forEach((taskButton) =>
    taskButton.addEventListener("click", editTask)
  );
}
...
function displayProjectPreview(i) {
  let ind = i;
  try {
    ind = Number(this.getAttribute("data-index-project-name"));
  } catch (error) {}
...
function displayTasks(ind) {
  const tasksList = document.querySelector(".tasks");
  tasksList.innerHTML = myProjects[ind].tasks
    .map((task, i) => {
      return `
    <li> 
     <div class="task-due-date">       
         <input type="checkbox" id="task${i}" ${task.done ? "checked" : ""}>
         <label for="task${i}" data-project="${ind}" data-index="${i}"> ${
        task.title
      } </label>
